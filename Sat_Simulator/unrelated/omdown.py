...
for idx in range(len(names)):
    ...
    packets = np.array([int(k)*255 for k in mapNode.values()])
    vals.append(packets)

    print("Median",np.percentile(packets,50))
    print("90th",np.percentile(packets,90))
    print("100th",np.percentile(packets,90))


sns.ecdfplot(vals, ax=ax, label=names)

ax.tick_params(axis='both', which='major',labelsize=15)
ax.legend(fontsize=18, loc='lower right')
ax.set_xlabel('Downlink Throughput per Sat (Bytes)',fontsize=15)
ax.set_ylabel('CDF',fontsize=15)
plt.tight_layout()
plt.savefig("omthrough.png")  